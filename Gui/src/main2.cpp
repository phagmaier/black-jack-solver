...
void check_dead(Vector2 &pos, Rectangle *rec1, Rectangle *rec2, 
                bool *bools1, bool *bools2, std::pair<int,int> &card_clicked, 
...
void draw_dead_cards(Rectangle *recs1, Rectangle *recs2, bool *bools1, bool *bools2,Font &font){
  static const char *chars[NUM_CARDS] = {"A", "2", "3", "4", "5", "6", "7", "8", "9", "T"};
  Rectangle tmp;
  Color color;
  for (int i=0;i<NUM_CARDS;++i){
    tmp = recs1[i];
    color = bools1[i] ? YELLOW : LIGHTGRAY;
    ...
    //input box
    //DrawRectangleLines(tmp.x+10,tmp.y+tmp.width -10,tmp.width/2,tmp.height/2,BLACK);
    //DrawRectangle(tmp.x+10,tmp.y+tmp.width -10,tmp.width/2,tmp.height/2,color);
...
void init_dead_cards(Rectangle *recs1, Rectangle *recs2, bool *bools1, bool *bools2, int *counts1, int *counts2){
  //int gap = 30;
  //int padding = 30;
  for (int i=0;i<NUM_CARDS;++i){
    bools1[i] = false;
...
void init_cads(Rectangle *p, Rectangle *d, bool *bools_p, bool *bools_d){
  ...
  for (int row=0;row<num_rows;++row){
    for (int col=0;col<num_cols;++col){
      ...
      std::cout << "COL X: " << (float)c_w * (float)col << "\n";
      std::cout << "Col Width is: "<< c_w << "\n";
      //d[count] = Rectangle{static_cast<float>(c_w * (col+num_cols) + space * (col+1)) + padding, static_cast<float>(c_h * row), c_w, c_h};;
...
int main(){
  ...
  init_dead_cards(dead_Recs1,dead_Recs2,dead_clicked1,dead_clicked2,counts1,counts2);
  ...
  while (!WindowShouldClose()){
    ...
    if (clicked_done){
      ...
      while (!dead_done){
        ...
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
          ...
          /*
           *Vector2 &pos, Rectangle *rec1, Rectangle *rec2, 
            bool *bools1, bool *bools2, std::pair<int,int> &card_clicked, 
            bool &clicked, Rectangle &done, bool &is_done, 
            int *counts1, int *counts2
           */
          check_dead(pos,dead_Recs1,dead_Recs2, dead_clicked1,dead_clicked2,card_clicked,get_input,dead_done_rec,dead_done);
          //call_function;
       }
        BeginDrawing();
        ClearBackground(RAYWHITE);
        draw_dead_cards(dead_Recs1,dead_Recs2,dead_clicked1,dead_clicked2,font40);
        EndDrawing();
      }
      break;
    }
    ...
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
      Vector2 pos= GetMousePosition();
      /*
       *check_click(const Vector2 &pos, const Rectangle *p, const Rectangle *d, bool *bool_p, 
                 bool *bool_d, const Rectangle &decks, const Rectangle &done, 
                 bool &bool_decks, bool &bool_done, std::pair<int,int>&p1_hand, int &dealer_hand, const int num_decks){
